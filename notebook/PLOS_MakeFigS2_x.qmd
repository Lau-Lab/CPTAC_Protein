...
for( tumor in paste0('tumor', 8)){
    for (method in c('elastic', 'forest')){
        for (feature in c('single', 'corum', 'stringhi')){
...
# Pretty naming
df_all <- df_all |> dplyr::left_join(data.frame(feature=c('single', 'corum', 'stringhi'),
                                                 featurename=c('Single', 'CORUM', 'STRING 800')))
...
df_all$featurename <- factor(df_all$featurename, levels=c('Single', 'CORUM', 'STRING 800'))
...
ggsave(plot=g, filename="PLOS_FigS2B.pdf", width=12, height=9, useDingbats=F)
```
